...
<script>
import {delObjs, fetchLogList} from '@/api/daemon/sys-job'
...
export default {
  ...
  methods: {
    ...
    handleDelLogs() {
      if (this.$refs.crudLog.tableSelect.length === 0) {
        this.$message.error('请选择日志后进行删除')
        return false
      }
      const logIds = []
      for (const row of this.$refs.crudLog.tableSelect) {
        logIds.push(row.jobLogId)
      }
      delObjs(logIds).then(() => {
        this.$notify.success('删除成功')
        this.$refs.crudLog.toggleSelection()
        this.pageLog.currentPage = 1
        this.getJobLogList(this.pageLog)
      })
    }
