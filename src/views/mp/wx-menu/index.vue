          ...
          <div class="left">
            ...
            <div class="weixin-menu menu_main clearfix">
              <div v-for="(item, i) of menu.button" :key="i" class="menu_bottom">
                <!-- 一级菜单 -->
                <div class="menu_item el-icon-s-fold" :class="{'active': isActive === i}" @click="menuFun(i,item)">
                  {{ item.name }}
                </div>
                <!--以下为二级菜单-->
          ...
          <div v-if="showRightFlag" class="right">
            <div class="configure_page">
              ...
              <div v-if="showConfigurContent">
                ...
                <div v-if="tempObj.type === 'miniprogram'" class="configur_content">
                  <div class="applet">
                    <span>小程序的appid：</span>
                    <el-input
                ...
                <div v-if="tempObj.type === 'view_limited'" class="configur_content">
                  <el-row>
                    <div v-if="tempObj && tempObj.content && tempObj.content.newsItem" class="select-item">
                      <WxNews :obj-data="tempObj.content.newsItem"></WxNews>
                      <el-row class="ope-row">
                        <el-button type="danger" icon="el-icon-delete" circle @click="deleteTempObj"></el-button>
                      </el-row>
                    </div>
                    <div v-if="!tempObj.content || !tempObj.content.newsItem">
                      <el-row>
                    ...
                    <el-dialog title="选择图文" :visible.sync="dialogNewsVisible" width="90%">
                      <WxMaterialSelect
                        :obj-data="{repType:'news',appId: checkAppId}"
                        @selectMaterial="selectMaterial"></WxMaterialSelect>
                    </el-dialog>
                  </el-row>
                </div>
                <div v-if="tempObj.type === 'click' || tempObj.type === 'scancode_waitmsg'" class="configur_content">
                  <WxReplySelect v-if="hackResetWxReplySelect" :obj-data="tempObj"></WxReplySelect>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    selectMaterial(item) {
      if (item.content.newsItem.length > 1) {
      ...
      item.content.newsItem = item.content.newsItem.slice(0, 1)
